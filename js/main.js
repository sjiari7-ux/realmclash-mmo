...
document.addEventListener('click', async (e)=>{
  ...
  if(action==='create-character'){
    ...
    S.char = newCharacter(MY_ID, uname, st.classId); // tutorialSeen:false — first-time walkthrough shows on 'home'
    setScreen('home');
    saveCharacter(S.char); // fire-and-forget: don't hold up entering the game on the network write
    return;
  }
  if(action==='tutorial-dismiss'){ S.char.tutorialSeen = true; S.tutorialIdx = 0; await persist(); return; }
  if(action==='tutorial-next'){ S.tutorialIdx = (S.tutorialIdx||0) + 1; render(); return; }
...
function migrateCharacter(c){
  // Existing characters predate the tutorial — they've already played, so they should
  // never see the first-time walkthrough retroactively. Only newCharacter() sets this false.
  if(c.tutorialSeen===undefined) c.tutorialSeen = true;
